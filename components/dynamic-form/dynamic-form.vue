			...
			<view class="value flex-end">
				...
				<template v-if="item.type=='picker'">
					<view class="picker flex-end">
						...
						<image class="icon" src="../../static/images/dynamic-form/icon-right.png"></image>
					</view>
				</template>
				...
				<template  v-if="item.type=='uploadMedia'">
					<view class="upload flex-end" @click="upload(item)">
						<template v-if="form[item.key]">
							已上传
							<image class="icon" src="../../static/images/dynamic-form/add-icon-2.png"></image>
						</template>
						<template v-else>
							请上传
							<image class="icon" src="../../static/images/dynamic-form/add-icon-2.png"></image>
				...
				<template v-if="item.type == 'pictureGroup'">
					<view class="pictureGroup flex-start">
						<view class="child flex-center" v-for="(child, childIndex) in form[item.key]" :key="childIndex">
							...
							<image @click="deleteImage(item, child)" src="../../static/images/dynamic-form/close.png" class="close"></image>
						</view>
						<view v-if="!form[item.key] || (form[item.key].length) < (item.max || 1)" class="add flex-center" @click="uploadPicture(item)">
							<image src="../../static/images/dynamic-form/add-icon-2.png" mode=""></image>
							<view>请上传</view>
						</view>
					</view>
				</template>
		...
		methods: {
			...
			uploadPicture(item) {
				...
				uni.chooseMedia({
					...
					fail(error) {
						console.error(error,'--dynamic-form')
					}
				})
			},
